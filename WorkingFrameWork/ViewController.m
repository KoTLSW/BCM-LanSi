...
@interface ViewController()<NSTextFieldDelegate>
{
    ...
    IBOutlet NSTextField *netState_TF;
    ...
    IBOutlet NSButton *isAutoScanBtn;
...
- (void)viewDidLoad {
   ...
   [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(selectNullTestNotiM:) name:@"NULLTEST" object:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(selectScanMode_LimitNoti:) name:@"AutoScanButton_Notification" object:nil];
...
-(void)reloadPlist
{
    ...
    action1.isCancel=NO;
    action1.isAutoScan=isAutoScanBtn.state;
    [action1 setFoldDir:foldDir];
    ...
    action2.isCancel=NO;
    action2.isAutoScan=isAutoScanBtn.state;
    ...
    action3.isCancel=NO;
    action3.isAutoScan=isAutoScanBtn.state;
    ...
    action4.isCancel=NO;
    action4.isAutoScan=isAutoScanBtn.state;
...
- (IBAction)stopScan:(NSButton *)sender
{
    if ([sender.title isEqualToString:@"暂停扫描"])
    {
        sender.title=@"开始扫描";
        [[NSNotificationCenter defaultCenter] postNotificationName:@"stopScan" object:@""];
    }
    else
    {
        sender.title=@"暂停扫描";
        [[NSNotificationCenter defaultCenter] postNotificationName:@"startScan" object:@""];
    }
}
...
-(void)selectScanMode_LimitNoti:(NSNotification *)noti
{
    isAutoScanBtn.enabled=YES;
}
        ...
        if (index == 3)
        {
            ...
            controlLog=1;
             dispatch_sync(dispatch_get_main_queue(), ^{
                ...
                {
                    [self UpdateTextView:@"index=3,SN检测OK" andClear:NO andTextView:Log_View];
...
#pragma mark-------------//index=5,双击启动/点击界面start按钮---》气缸动作---->start ok
 ...
 #pragma mark-------------//index=6,抛出开始测试信号
        if (index == 6) {
            ...
            [NSThread sleepForTimeInterval:0.1];
            dispatch_async(dispatch_get_main_queue(), ^{
                [Status_TF setStringValue:@"index=6,Testing..."];
...
- (IBAction)isAutoScanBtn:(NSButton *)sender
{
    if (sender.state==0)
    {
        action1.isAutoScan=NO;
        action2.isAutoScan=NO;
        action3.isAutoScan=NO;
        action4.isAutoScan=NO;
        
        [self UpdateTextView:@"手动扫码模式" andClear:NO andTextView:Log_View];

    }
    
    else if (sender.state==1)
    {
        action1.isAutoScan=YES;
        action2.isAutoScan=YES;
        action3.isAutoScan=YES;
        action4.isAutoScan=YES;
        [self UpdateTextView:@"自动扫码模式" andClear:NO andTextView:Log_View];
    }
}
    ...
    NSURLSessionTask *task = [session dataTaskWithRequest:theRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error)
                              {
                                  ...
                                  {
                                      [self saveSFCLogWithString:[NSString stringWithFormat:@"SN %@ 上传失败！",sn_Upload] FileName:@"SFC_LOG"];
                                      [self saveSFCLogWithString:[NSString stringWithFormat:@"SN %@ 上传失败！",sn_Upload] FileName:[NSString stringWithFormat:@"%@SFC_LOG",[[GetTimeDay shareInstance] getCurrentMonth]]];
