...
-(void)startScan:(NSNotification *)noti
{
    [scanport Close];
...
-(void)stopScan:(NSNotification *)noti
{
    [scanport WriteLine:@"LOFF"];
    [NSThread sleepForTimeInterval:0.5];
    [scanport ReadExisting];
    index=1000;
    NSLog(@"暂停扫描");
...
#pragma mark--------index = 1001:连接扫码器
        if (index == 1001)
        {
            ...
            else
            {
                ...
                {
                    [self UpdateTextView:@"index=1,扫码器连接失败" andClear:NO andTextView:self.Log_View];
                }
            }
        }
...
#pragma mark--------index=1连接LCR表4980 和 静电仪器2987A
        if (index == 1) {
...
#pragma mark--------index = 1002:扫描SN
        if (index == 1002)
        {
           ...
           if ([scanport IsOpen])
           {
            ...
            if ([_dut_sn containsString:@"ERROR"] &&  [_dut_sn containsString:@"\n"])
            {
        ...
        {
            if (_dut_sn.length < 21 && _dut_sn.length > 17)
            {
        ...
        {
            [self UpdateTextView:@"index=3,SN获取失败" andClear:NO andTextView:self.Log_View];
        }
